...
public class TemplateDef implements Definition 
{
	public TemplateDef() 
	{
	}
	...
	public synchronized DefResultSet loadDefinitionData(LoginEmployee loginEmployee) throws Exception 
	{
		...
		{
			glTemplateBeans = new Template[v.size()];		
			v.toArray(glTemplateBeans);		
			defResultSet.setGlTemplateBeans(glTemplateBeans);
		}
	...
	private void execute() throws MappingException, IOException, Exception 
    {
        ...
        {	
			...
			{
				query = session.createQuery(DefQueries.findTemplateQuery);
			} else if (loginEmployee.getFilterBy() != null && (loginEmployee.getFilterBy().equals(AppConstants.filterForOtherScreenUsage)))
			{
				query = session.createQuery(DefQueries.findTemplatePostedQuery);
			}

            List<Template> resultlist = query.list();
            System.out.println("Templates data found: " + resultlist.size());
            for(Template bean: resultlist) 
            {
    			v.add(bean);
            }
	...
	private Vector<Template> v = new Vector<Template>();
