...
public class CityDef implements Definition 
{
	...
	public synchronized DefResultSet loadDefinitionData(LoginEmployee loginEmployee) throws Exception 
	{
		...
		{
			cityBeans = new City[v.size()];		
			v.toArray(cityBeans);		
			defResultSet.setCityBeans(cityBeans);
		}
	...
	private void execute() throws MappingException, IOException, Exception 
    {
        ...
        {	
			...
			{
				query = session.createQuery(DefQueries.findCityQuery);
			} else if (loginEmployee.getFilterBy() != null && (loginEmployee.getFilterBy().equals(AppConstants.filterForOtherScreenUsage)))
			{
				query = session.createQuery(DefQueries.findCityPostedQuery);
			}
        	
            List<City> resultlist = query.list();
            System.out.println("city data found: " + resultlist.size());
            for(City bean: resultlist) 
            {
    			v.add(bean);
            }
	...
	private Vector<City> v = new Vector<City>();
